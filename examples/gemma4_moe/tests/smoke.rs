//! Offline per-plan proofs for the gemma4_moe crate loop (no HF
//! download) — tiny dims keep the role split (layer 0 sliding hd4/2kv,
//! layer 1 full hd8/1kv partial rotary) and 4-expert top-2 routing:
...
use gemma4_moe::model::Gemma4Dims;
use gemma4_moe::{DecodeStep, Decoder, weights};
...
fn tiny_decode_loop_is_deterministic_and_advances_the_cache() {
    let dims = Gemma4Dims::tiny();
    let max_seq = 4usize;
    // Heterogeneous pool: check each layer at its own kv width.
