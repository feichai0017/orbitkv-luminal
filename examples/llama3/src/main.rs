use llama3::{Llama3RunConfig, run_llama3};

const USAGE: &str = "\
llama3 — Meta-Llama-3-8B-Instruct as pure logical ops on the SSA reference runtime

USAGE: cargo run --release -p llama3 [-- OPTIONS]
  ...
  --repo <id>         HuggingFace repo (default NousResearch/Meta-Llama-3-8B-Instruct)
  --random-weights    skip the download; deterministic fake parameters
  --help              this text";

fn main() {
    let mut config = Llama3RunConfig::default();
    ...
    if let Err(err) = run_llama3(config) {
        eprintln!("error: {err}");
        std::process::exit(1);
    }
}
