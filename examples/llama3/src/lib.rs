//! Llama-3-8B-Instruct on the native ladder — step-invariant decode
//! over the position-slots cache driver (see the qwen exemplar for the
//! one-search-N-executes contract). This example exercises
//! [`luminal_nn::KvCachePool`] + [`luminal_nn::PositionSlots`]; the
//! paged_llama3 example exercises the page-table driver over the same
//! pool structure.
...
impl Default for Llama3RunConfig {
    fn default() -> Self {
        Self {
            repo_id: "NousResearch/Meta-Llama-3-8B-Instruct".to_string(),
...
impl DecodeStep {
    pub fn build(dims: &Llama3Dims, max_seq: usize) -> Self {
...
pub fn run_llama3(config: Llama3RunConfig) -> Result<(), Box<dyn Error>> {
