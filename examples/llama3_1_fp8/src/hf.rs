...
fn stored_tensor_bf16(name: &str, tensor: &safetensors::tensor::TensorView) -> StoredTensor {
    ...
    // The fp8 passthrough rule: E4M3FN weights and their F32 scalar
    // scales are the MODEL — bytes pass through untouched.
    if tensor.dtype() == Dtype::F8_E4M3 || name.ends_with("_scale") {
        return StoredTensor {
            shape,
            dtype: tensor.dtype(),
...
/// Combines sharded safetensors into a single file, UNFUSED, with the
/// fp8 passthrough rule: E4M3FN tensors and their F32 scales pass
/// through BYTE-IDENTICAL (quantization is model definition — the
/// codes are the weights); "*norm*" weights stay F32; everything else
/// (embeddings, lm_head) converts to bf16. The reference runtime
/// stages numerics as f32 and fp8 as native code buffers (weights.rs).
///
...
pub fn combine_safetensors_to_bf16(
    ...
    let output_path = model_dir.join("model_combined_fp8_unfused_v1.safetensors");
...
/// - tokenizer.json
/// - model_combined_fp8_unfused_v1.safetensors
