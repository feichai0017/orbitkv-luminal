//! nvidia/Llama-3.1-8B-Instruct-FP8 on the native ladder — the fp8
//! zoo example: E4M3FN weights staged as native code buffers, the
//! quantize/dequant chain in model text, and the llama3-type rope
//! frequency ramp. Position-slots cache driver; step-invariant decode.
...
const EOS_TOKEN: u32 = 128_009; // <|eot_id|>
const STOP_TOKEN: u32 = 128_001; // <|end_of_text|>

pub struct Fp8RunConfig {
    pub repo_id: String,
    pub prompt: String,
...
impl Default for Fp8RunConfig {
    fn default() -> Self {
        Self {
            repo_id: "nvidia/Llama-3.1-8B-Instruct-FP8".to_string(),
...
impl DecodeStep {
    pub fn build(dims: &Fp8Dims, max_seq: usize) -> Self {
...
impl Decoder {
    pub fn start(
        ...
        // The llama3-type frequency ramp — THE 3.1 fidelity delta the
        // parked example omitted.
        let (cos_table, sin_table) = luminal_nn::rope_tables_llama3_scaled(
            &positions,
            dims.head_dim,
            dims.rope_theta,
            dims.rope_factor,
            dims.rope_low_freq_factor,
            dims.rope_high_freq_factor,
            dims.rope_original_max,
        );
...
pub fn run_llama3_1_fp8(config: Fp8RunConfig) -> Result<(), Box<dyn Error>> {
    let mut dims = Fp8Dims::llama3_1_8b_fp8();
    ...
    let prompt_tokens: Vec<u32> = match &tokenizer {
        Some(tokenizer) => tokenizer
            .encode(llama3_chat_prompt(&config.prompt).as_str(), false)
            .map_err(|err| err as Box<dyn Error>)?
            .get_ids()
            .to_vec(),
        None => vec![1, 2, 3],
