use gemma3::{Gemma3RunConfig, run_gemma3};

const USAGE: &str = "\
gemma3 — unsloth/gemma-3-4b-it (text tower) as pure logical ops

USAGE: cargo run --release -p gemma3 [-- OPTIONS]
  --prompt <text>     user prompt (llama3 chat template applied)
  --layers <n>        transformer layers to instantiate, 1..=34 (default 1)
  --tokens <n>        tokens to generate (default 8)
  --max-seq <n>       cache slots / maximum sequence length (default 64)
  --repo <id>         HuggingFace repo (default unsloth/gemma-3-4b-it)
  --random-weights    skip the download; deterministic fake parameters
  --help              this text";

fn main() {
    let mut config = Gemma3RunConfig::default();
    ...
    if let Err(err) = run_gemma3(config) {
        eprintln!("error: {err}");
        std::process::exit(1);
    }
}
