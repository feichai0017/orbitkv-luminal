//! MiniGemma4Moe fidelity: the graph's one decode step must match a
//! scalar reference (trusted-validator doctrine). Moved out of
...
fn mini_moe_family(softcap: f32) {
    const VOCAB: usize = 5;
    const D: usize = 4;
    const E: usize = 2;
    const NH: usize = 2;
    ...
    let ref_logits: Vec<f32> = (0..VOCAB)
        .map(|v| (0..D).map(|i| x2[i] * embed_w[v * D + i]).sum::<f32>())
        .map(|v: f32| (v / softcap).tanh() * softcap)
        .collect();
