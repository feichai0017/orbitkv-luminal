...
fn build_paged_attention_graph_with_mask_and_cache_provenance(
    ...
    let q_rope = cx.named_tensor("q_rope", ('s', hidden), DType::F32);
    let k_rope = cx.named_tensor("k_rope", ('s', kv_dim), DType::F32);
    let v_new = cx.named_tensor("v_new", ('s', kv_dim), DType::F32);
    let k_cache = cx
    ...
    let v_cache = cx
        ...
        .persist();
    let scatter_idx = cx.named_tensor("scatter_idx", 's', luminal::dtype::DType::Int);
    let gather_idx = cx.named_tensor("gather_idx", 'c', luminal::dtype::DType::Int);
    let q_pos = cx.named_tensor("q_pos", 's', luminal::dtype::DType::Int);
